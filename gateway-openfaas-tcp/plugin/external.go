...
func (s ExternalServiceQuery) GetReplicas(serviceName, serviceNamespace string) (scaling.ServiceQueryResponse, error) {
	...
	if err != nil {
		...
		if res.StatusCode == http.StatusOK {
			...
			//log.Printf("[External.go] if branch: GetReplicas [%s.%s] took: %fs", serviceName, serviceNamespace, time.Since(start).Seconds())

		} else {
			//log.Printf("[External.go] else branch: GetReplicas [%s.%s] took: %fs, code: %d\n", serviceName, serviceNamespace, time.Since(start).Seconds(), res.StatusCode)
