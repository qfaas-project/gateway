...
func MakeForwardingProxyHandler(proxy *types.HTTPClientReverseProxy,
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//fmt.Printf("[handlers/forwarding_proxy] originalURL %s, requestURL %s\n", originalURL, requestURL)
...
func buildUpstreamRequest(r *http.Request, baseURL string, requestURL string) *http.Request {
	...
	if strings.Contains(url, "openfaas-fn.svc.cluster.local") {
		isFunctionReq = true
	}

	// for TLS connetcion: http to https
	if isFunctionReq {
...
func (f FunctionAsHostBaseURLResolver) Resolve(r *http.Request) string {
	...
	//log.Printf("[forwarding_proxy] Resolver, svcName: %s, http://%s%s:%d\n", svcName, svcName, suffix, watchdogPort)
