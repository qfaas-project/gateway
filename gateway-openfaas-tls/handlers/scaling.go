...
func MakeScalingHandler(next http.HandlerFunc, scaler scaling.FunctionScaler, config scaling.ScalingConfig, defaultNamespace string) http.HandlerFunc {
	//log.Printf("[handlers/scaling] create MakeScalingHandler")

	return func(w http.ResponseWriter, r *http.Request) {

		//log.Printf("[handlers/scaling] call MakeScalingHandler")

		//log.Printf("[handlers/scaling] r.URL.String: %s", r.URL.String())
		...
		if res.Available {
			//log.Printf("[handlers/scaling] call MakeScalingHandler: Available")
			next.ServeHTTP(w, r)
			return
		}
