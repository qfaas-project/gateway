# QFaaS Note for gateway-openfaas-tls

Original OpenFaaS `gateway` component with TLS 1.2 support
...
**Info**
 ...
 - Please check [main.go](main.go) and [types/proxy_client.go](types/proxy_client.go) to learn more about TLS client
 - [certs](certs/)  contains our self-signed certificate for prototype testing propuse
  

## Build Instruction

```shell script
./build.sh v3.0 openfaas
docker images
# You should find a docker image called openfaas/gateway-openfaas-tls:v3.0
# You can use any version number you like instead of 'v3.0'
```

Modification

 - main.go: use HTTPS Client for function proxy
 - types/proxy_client.go: create a TLS client
 - HTTP to HTTPs url
