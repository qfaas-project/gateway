...
func main() {
	...
	// QFaaS
	fmt.Printf("OpenFaaS Gateway TLS version: v3.0\n")
	...
	TLSReverseProxy := types.NewHTTPSClientReverseProxy(config.FunctionsProviderURL,
		config.UpstreamTimeout,
		config.MaxIdleConns,
		config.MaxIdleConnsPerHost)
	...
	if config.DirectFunctions {
		log.Printf("config.DirectFunctions is True")
	...
	faasHandlers.Proxy = handlers.MakeForwardingProxyHandler(TLSReverseProxy, functionNotifiers, functionURLResolver, functionURLTransformer, nil)
