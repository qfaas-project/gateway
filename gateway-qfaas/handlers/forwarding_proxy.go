...
func buildUpstreamRequest(r *http.Request, baseURL string, requestURL string) *http.Request {
	...
	// QFaaS
	// for QUIC connetcion: http to https
	if isFunctionReq {
	...
	// QFaaS
	// replace function GET to QUIC GET0RTT
	if isFunctionReq && upstreamReq.Method == http.MethodGet {
		upstreamReq.Method = http3.MethodGet0RTT
	}
