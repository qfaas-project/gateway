...
func main() {
	...
	fmt.Println("metadataQuery: ", metadataQuery)
	...
	// QFaaS
	fmt.Printf("QFaaS Gateway version: v3.0\n")

	log.Printf("config.FunctionsProviderURL: %s", config.FunctionsProviderURL)
	...
	// QFaaS
	quicReverseProxy := types.NewQUICClientReverseProxy(config.FunctionsProviderURL,
		config.UpstreamTimeout,
		config.MaxIdleConns,
		config.MaxIdleConnsPerHost)

	fmt.Println("QUIC Proxy: ", quicReverseProxy)
	fmt.Println("reverseProxy: ", reverseProxy)
	...
	faasHandlers.Proxy = handlers.MakeForwardingProxyHandler(quicReverseProxy, functionNotifiers, functionURLResolver, functionURLTransformer, nil)
